...
def render_page(title: str, content: str, active: str = "dashboard"):
    ...
    theme = get_theme_vars(theme_name)
    theme_toggle = "dark" if theme_name == "blue" else "blue"
    template = """
    <!doctype html>
    <html lang="ru">
    <head>
        ...
        <style>
            :root { --bg1: {{ theme.bg1 }}; --bg2: {{ theme.bg2 }}; --bg3: {{ theme.bg3 }}; --text: {{ theme.text }}; --muted: {{ theme.muted }}; --line: {{ theme.line }}; --blue: {{ theme.blue }}; --blue2: {{ theme.blue2 }}; --radius:20px; }
            * { box-sizing:border-box; }
            ...
            .sidebar { background:{{ theme.sidebar }}; backdrop-filter:blur(10px); border-right:1px solid rgba(148,163,184,.10); padding:24px 18px; }
        ...
        <div class="layout">
            <aside class="sidebar">
                ...
                <nav class="nav">
                    ...
                    <a href="/">🏠 На главную</a>
                    <a href="/theme/{{ theme_toggle }}?next={{ request.path }}">🎨 Переключить тему</a>
    ...
    """
    return render_template_string(template, title=title, content=content, active=active, message=message, theme=theme, theme_toggle=theme_toggle, request=request, logo=logo_markup())
...
def render_staff_page(title: str, content: str, active: str = "dashboard"):
    ...
    <head>
        ...
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <style>
            :root { --bg1: {{ theme.bg1 }}; --bg2: {{ theme.bg2 }}; --bg3: {{ theme.bg3 }}; --text: {{ theme.text }}; --muted: {{ theme.muted }}; --line: {{ theme.line }}; --blue: {{ theme.blue }}; --radius:20px; }
            * { box-sizing:border-box; }
            ...
            .layout { display:grid; grid-template-columns:280px 1fr; min-height:100vh; }
            .sidebar { background:{{ theme.sidebar }}; padding:24px 18px; border-right:1px solid rgba(148,163,184,.10); }
            .brand h1 { margin:0 0 8px 0; font-size:22px; }
            .brand p { margin:0 0 24px 0; color:var(--muted); font-size:14px; }
            .nav { display:grid; gap:10px; }
            .nav a { display:block; padding:14px; border-radius:14px; color:#dbeafe; text-decoration:none; border:1px solid transparent; }
    ...
    """
    return render_template_string(template, title=title, content=content, active=active, message=message, user=user, theme=theme, theme_toggle=theme_toggle, request=request, logo=logo_markup())
...
def public_index():
    ...
    <body>
        <div class="shell">
            <div class="hero">
                <div style="display:flex;justify-content:center;margin-bottom:16px;"><img src="/static/img/sk_logo.png" alt="Герб СК" style="width:92px;height:92px;object-fit:contain;filter:drop-shadow(0 0 12px rgba(111,177,255,.30)) drop-shadow(0 0 26px rgba(111,177,255,.18));" onerror="this.style.display='none'"></div>
                <h1>Следственный Комитет</h1>
                <p>Следственный комитет РФ: защита гражданским - наша цель.</p>
            </div>
            <div class="grid">
                <div class="card">
                    <h3>🔐 Мой раздел</h3>
...
@app.route("/appeal/<path:number>/edit", methods=["GET", "POST"])
@login_required
