...
def render_page(title: str, content: str, active: str = "dashboard"):
    ...
    <head>
        ...
        <style>
            :root {
                --bg1:#08111f; --bg2:#0f172a; --bg3:#111827; --card:#182235; --line:#334155;
                --text:#e5e7eb; --muted:#94a3b8; --blue:#2563eb; --blue2:#1d4ed8; --shadow:0 16px 35px rgba(0,0,0,.30); --radius:20px;
            }
            * { box-sizing:border-box; }
            body { margin:0; padding:0; font-family:Arial,sans-serif; color:var(--text);
                background: radial-gradient(circle at top right, rgba(37,99,235,0.12), transparent 20%),
            ...
            .sidebar { background:rgba(10,16,30,0.88); backdrop-filter: blur(10px); border-right:1px solid rgba(148,163,184,.08); padding:24px 18px; position:sticky; top:0; height:100vh; }
            ...
            .btn { display:inline-block; padding:11px 16px; border:none; border-radius:12px; background:var(--blue); color:white; font-weight:bold; cursor:pointer; text-decoration:none; box-shadow:var(--shadow); }
            ...
            .card { background:linear-gradient(180deg, rgba(24,34,53,.98), rgba(19,28,44,.98)); border:1px solid rgba(148,163,184,.08); border-radius:var(--radius); padding:18px; box-shadow:var(--shadow); margin-bottom:20px; }
...
def public_index():
    ...
    <head>
        ...
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <style>
            :root { --bg1:#08111f; --bg2:#0f172a; --bg3:#111827; --text:#e5e7eb; --muted:#94a3b8; --blue:#2563eb; --blue2:#1d4ed8; --shadow:0 16px 35px rgba(0,0,0,.30); --radius:24px; }
            * { box-sizing:border-box; }
            ...
            .card { background:linear-gradient(180deg, rgba(24,34,53,.98), rgba(19,28,44,.98)); border:1px solid rgba(148,163,184,.08); border-radius:var(--radius); padding:24px; box-shadow:var(--shadow); }
    ...
    <body>
        <div class="shell">
            <div class="hero">
                ...
                <p>Единый портал для руководства, сотрудников и обращений напрямую руководителю.</p>
            </div>
            <div class="grid">
                <div class="card">
                    <h3>🔐 Мой раздел</h3>
                    <p>Вход в закрытую административную панель. Дашборд, обращения, кадры, аналитика, дисциплина и управление системой.</p>
                    <a class="btn" href="/login">Открыть раздел</a>
                </div>
                <div class="card">
                    <h3>👥 Раздел сотрудников</h3>
                    <p>Кабинет сотрудника: вход, личные данные, служебные действия и взаимодействие через веб-портал.</p>
                    <a class="btn" href="/staff-login">Перейти</a>
                </div>
                <div class="card">
                    <h3>🛂 Запрос доступа</h3>
                    <p>Подать заявку на доступ к разделу сотрудников по Discord ID и дождаться одобрения руководителем.</p>
...
def request_access():
    ...
    if request.method == "POST":
        ...
        if not discord_id_text.isdigit():
            ...
            approved_user = None
            try:
                from database import get_web_user_by_discord_id
            ...
            if approved_user:
                ...
                with get_connection() as conn:
                    with conn.cursor() as cur:
                        ...
                        """, (discord_id, fio, department, position, reason, "Новая", __import__("datetime").datetime.now().strftime("%d.%m.%Y %H:%M:%S")))
                    conn.commit()
    ...
    <head>
        ...
        <style>
            ...
            .card { width:100%; max-width:680px; background:linear-gradient(180deg, rgba(24,34,53,.98), rgba(19,28,44,.98)); border:1px solid rgba(148,163,184,.08); border-radius:24px; padding:26px; box-shadow:0 16px 35px rgba(0,0,0,.30); }
...
def staff_login():
    ...
    <head>
        ...
        <style>
            body { margin:0; font-family:Arial,sans-serif; background:linear-gradient(135deg,#08111f,#0f172a,#111827); color:#e5e7eb; min-height:100vh; display:flex; align-items:center; justify-content:center; padding:24px; }
            .card { width:100%; max-width:560px; background:linear-gradient(180deg, rgba(24,34,53,.98), rgba(19,28,44,.98)); border:1px solid rgba(148,163,184,.08); border-radius:24px; padding:26px; box-shadow:0 16px 35px rgba(0,0,0,.30); }
            h1 { margin:0 0 12px 0; }
            p { color:#94a3b8; line-height:1.6; }
    ...
    <body>
        <div class="card">
            ...
            <p>Используйте Discord ID и пароль, который был выдан после одобрения заявки.</p>
            {% if error %}<div class="error">{{ error }}</div>{% endif %}
            <form method="post">
                <input type="text" name="discord_id" placeholder="Discord ID" required>
...
def staff_dashboard():
    ...
    <head>
        ...
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <style>
            body { margin:0; font-family:Arial,sans-serif; background:linear-gradient(135deg,#08111f,#0f172a,#111827); color:#e5e7eb; min-height:100vh; padding:24px; }
            .shell { max-width:980px; margin:0 auto; }
            .card { background:linear-gradient(180deg, rgba(24,34,53,.98), rgba(19,28,44,.98)); border:1px solid rgba(148,163,184,.08); border-radius:24px; padding:24px; box-shadow:0 16px 35px rgba(0,0,0,.30); margin-bottom:20px; }
            h1, h2 { margin-top:0; }
            p { color:#94a3b8; line-height:1.6; }
            .grid { display:grid; grid-template-columns:1fr 1fr; gap:16px; }
            .box { background:#0f172a; border:1px solid rgba(148,163,184,.08); border-radius:14px; padding:14px; }
            a.btn { display:inline-block; padding:12px 16px; border-radius:12px; background:#334155; color:white; text-decoration:none; font-weight:bold; }
    ...
    <body>
        <div class="shell">
            <div class="card">
                ...
                <p>Доступ в веб-портал одобрен. Следующим этапом сюда можно добавить документы, обращения и другие служебные функции.</p>
...
def login():
    ...
    <style>
        ...
        .login-card { width:100%; max-width:420px; background:linear-gradient(180deg, rgba(24,34,53,.98), rgba(19,28,44,.98)); border:1px solid rgba(148,163,184,.08); border-radius:24px; padding:26px; box-shadow:0 16px 35px rgba(0,0,0,.30); }
...
def dashboard():
    ...
    <div class="row-3">
        <div class="card">
            ...
            <div class="chart">
                {''.join([f'<div class="bar-row"><div>{label}</div><div class="bar"><span style="width:{width}%"></span></div><div>{value}</div></div>' for label, value, width in analytics])}
            </div>
        </div>
...
def access_requests():
    ...
    for item in requests_list:
        ...
        <tr>
            ...
            <td><a href="/access-request/{item.get("id")}">Открыть</a></td>
        </tr>
        """
...
def access_request_card(request_id):
    ...
    if item.get("status") == "Новая":
        actions = f"""
        <div class="toolbar">
            <form method="post" action="/access-request/{request_id}/approve">
                <button class="btn" type="submit">✅ Одобрить</button>
            </form>
            <form method="post" action="/access-request/{request_id}/reject">
    ...
    password_block = ""
    if item.get("approved_password"):
        password_block = f"<div class='box'><b>Выданный пароль:</b><br>{item.get('approved_password')}</div>"
...
def approve_access_request_route(request_id):
    ...
    return redirect(url_for("access_request_card", request_id=request_id, message="Заявка одобрена. Пароль отправлен ботом в ЛС, а также сохранён в карточке."))
...
def web_users():
    ...
    for item in users:
        ...
        <tr>
            ...
            <td>{item.get("department","")}</td>
            <td>{item.get("role","")}</td>
            <td>{"Да" if item.get("is_active") else "Нет"}</td>
            <td>{item.get("created_at","")}</td>
        </tr>
        """
    content = f"""
...
def appeals():
    ...
    appeals_filtered = filter_appeals(all_appeals, appeal_status=appeal_status, appeal_department=appeal_department, appeal_number=appeal_number, appeal_priority=appeal_priority)
...
def appeal_card(number):
    ...
    <div class="card"><h3>🕒 История обращения</h3><div class="timeline">{history_html or '<div class="timeline-item">История пуста</div>'}</div></div>
    """
...
def employees():
    ...
    employees_filtered = filter_employees(all_employees, department=employee_department, fio=employee_fio, status=employee_status)
...
def save_employee():
    ...
    upsert_employee_from_web(discord_id=discord_id, fio=fio, department=department, position=position, rank_name=rank_name, status=status, notes=notes)
...
def analytics():
    ...
    content = f"""
    <div class="row-2">
        <div class="card"><h3>📊 Статусы обращений</h3><img src="/static/{status_chart}" alt="Статусы обращений"></div>
        <div class="card"><h3>🏆 Топ сотрудников</h3><img src="/static/{employee_chart}" alt="Топ сотрудников"></div>
    </div>
    <div class="card"><h3>🏢 Подразделения</h3><img src="/static/{department_chart}" alt="Подразделения"></div>
    """
